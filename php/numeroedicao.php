...
body{
    ...
    background-image: url(arlequina.jpg);
}
}
.menu{
        padding-top: 6px;
...
.table-bg{
    background-color: rgba(191,6,92,0.7);
...
<header>
    ...
    <div class="m-5">
    <form name="search" action="busca.php">
        <input class="textpesquisa" name="edicao" placeholder="  Número da Edição" type="text">
        <button class="lupa" type="submit"><img src="lupa.png" /></button>
    </form>
    <br>
    <br>
  ...
  <thead>
    <tr>
        <th scope="col" name="numero_edicao">Número da Edição</th>
      <th scope="col" name="nome_revista">Nome da Revista</th>
      <th scope="col" name="colecao">Coleção</th>
      <th scope="col" name="ano">Ano</th>
    </tr>
  </thead>     
  <tbody>
<?php
...
$sql = $con->query("SELECT `id`, `nome_revista`, `colecao`, `numero_edicao`, `ano`, `caixa_guardada`, `disponibilidade` FROM `revista` where revista.disponibilidade = 1 ORDER BY numero_edicao ASC");
    while($user_data = mysqli_fetch_assoc($sql)){
        echo "<tr>";
        echo "<td>".$user_data['numero_edicao']."</td>";
