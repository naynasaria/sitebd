...
<header>
    ...
    <div class="m-5">
    <form name="search" action="buscacaixas.php">
        <input class="textpesquisa" name="edicao" placeholder="  Cor da Caixa" type="text">
  ...
  <thead>
    <tr>
        <th scope="col" name="caixa">Caixa</th>
...
$sql = $con->query("SELECT `id`, `nome_revista`, `colecao`, `numero_edicao`, `ano`, `caixa_guardada`, `disponibilidade` FROM `revista` where revista.disponibilidade = 1 ORDER BY caixa_guardada DESC");
    while($user_data = mysqli_fetch_assoc($sql)){
        echo "<tr>";
        echo "<td>".$user_data['caixa_guardada']."</td>";
