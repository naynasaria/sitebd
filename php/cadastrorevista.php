...
<header>
    ...
    <h2 class="title conta2">Cadastre a sua Revista</h2>
            <form class="formrevista" method="post"  action="conexaorevista.php">
                <input class="field" type="text" name="nome" placeholder="  Nome da Revista">
                <input class="field" type="text" name="colecao" placeholder="  Coleção da Revista">
                <input class="field" type="number" name="numedicao" placeholder="  Número de Edição">
                <input class="field" type="number" name="ano" placeholder="  Ano da Revista">

                <select class="tabela" name="revista">
                <option value="caixa">Selecione a Caixa</option>

                    <?php
                        $sql = $con->query("SELECT cor FROM `caixa` WHERE 1");
                            while($revista = $sql->fetch_array())
                        {
                            echo '<option value="'.$revista["cor"].'">'.$revista["cor"].'</option>';    
                        }
                    ?>    
                </select>
