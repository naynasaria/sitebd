...
<header>
    ...
    <br>
    <a href="realizaremprestimo.php"><input class="botaodevolucao" type="submit" action="realizaremprestimo.php" name="botao" value="Fazer Empréstimo"></a>
    <br>
    <section class="content">
        <div class="contato">
            <h3>Faça a sua Devolução</h3>
            <form class="form" method="POST" action="devolutiva.php">
                    
                <select class="tabela" name="revista" required>´
                <option value="caixa">Selecione a Revista</option>
                    <?php
                        $sql = $con->query("SELECT * FROM revista INNER JOIN emprestimo
                        ON revista.nome_revista=emprestimo.nome_revista");
                ...
                <option value="caixa">Selecione a Coleção</option>
                    <?php
                        $sql = $con->query("SELECT `colecao` FROM revista where revista.disponibilidade = 0");
